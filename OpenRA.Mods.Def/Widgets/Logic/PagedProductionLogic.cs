...
namespace OpenRA.Mods.Def.Widgets.Logic
{
	public class PagedProductionLogic : ChromeLogic
	{
		readonly ProductionPaletteWidget palette;
		readonly ContainerWidget tabs;
		readonly ContainerWidget pager;
		readonly World world;

		void SetupPageButton(OrderManager orderManager, ProductionTypeButtonWidget button)
		{
			if (button == null)
				return;
			...
			{
				//Now this is where I need to have it check the pager which is going to have ProductionTypeButtonWidget
				// Substring out the generic type, won't support putting "." anywhere else in the declaration, no way around this without making a new trait.
				foreach (ProductionTypeButtonWidget t in tabs.Children.OfType<ProductionTypeButtonWidget>())
					if (t.IsHighlighted())
						palette.CurrentQueue = queues.FirstOrDefault(q => q.Info.Type.Substring(0, q.Info.Type.IndexOf('.')) == t.ProductionGroup && q.Enabled);
		...
		void SetupProductionGroupButton(OrderManager orderManager, ProductionTypeButtonWidget button)
		{
			...
			// Classic production queues are initialized at game start, and then never change.
			var queues = world.LocalPlayer.PlayerActor.TraitsImplementing<ProductionQueue>()
				.Where(q => q.Info.Type.Substring(0,q.Info.Type.IndexOf('.')) == button.ProductionGroup)
				.ToArray();

			Action<bool> selectTab = reverse =>
			{
				//If no CurrentQueue just give me a damn queue already
		...
		public PagedProductionLogic(Widget widget, OrderManager orderManager, World world)
		{
			this.world = world;
			...
			tabs = widget.Get<ContainerWidget>("PRODUCTION_TYPES");
			pager = widget.Get<ContainerWidget>("PRODUCTION_PAGES");

			foreach (var i in tabs.Children)
				SetupProductionGroupButton(orderManager, i as ProductionTypeButtonWidget);

			foreach (var j in pager.Children)
				SetupPageButton(orderManager, j as ProductionTypeButtonWidget);
			...
			ticker.OnTick = () =>
			{
				...
				{
					// Select the first active production tab
					foreach (var b in tabs.Children)
					{
					...
					// Select the first active page 
					foreach (var b in pager.Children)
					{
						var button = b as ProductionTypeButtonWidget;
						if (button == null || button.IsDisabled())
							continue;
